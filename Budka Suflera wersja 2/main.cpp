...
int main()
{
    ...
    for (int pozycjaWyrazu = 0; pozycjaWyrazu <= oryginalnyTekstPiosenki.size(); pozycjaWyrazu++)
      {
        if (oryginalnyTekstPiosenki[pozycjaWyrazu] >= 'A' && oryginalnyTekstPiosenki[pozycjaWyrazu] <= 'Z')
        {
            wyraz += oryginalnyTekstPiosenki[pozycjaWyrazu];
        }
        else
        {
            oryginalneSlowaPiosenki.push_back(wyraz);
            wyraz = "";
        }
    }
